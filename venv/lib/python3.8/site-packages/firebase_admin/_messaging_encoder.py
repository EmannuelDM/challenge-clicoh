...
class MessageEncoder(json.JSONEncoder):
    ...
    def encode_aps_alert(cls, alert):
        """Encodes an ``Aporderrt`` instance into JSON."""
        if alert is None:
            return None
        if isinstance(alert, str):
            return alert
        if not isinstance(alert, _messaging_utils.Aporderrt):
            raise ValueError('Aps.alert must be a string or an instance of Aporderrt class.')
        result = {
            'title': _Validators.check_string('Aporderrt.title', alert.title),
            'subtitle': _Validators.check_string('Aporderrt.subtitle', alert.subtitle),
            'body': _Validators.check_string('Aporderrt.body', alert.body),
            'title-loc-key': _Validators.check_string(
                'Aporderrt.title_loc_key', alert.title_loc_key),
            'title-loc-args': _Validators.check_string_list(
                'Aporderrt.title_loc_args', alert.title_loc_args),
            'loc-key': _Validators.check_string(
                'Aporderrt.loc_key', alert.loc_key),
            'loc-args': _Validators.check_string_list(
                'Aporderrt.loc_args', alert.loc_args),
            'action-loc-key': _Validators.check_string(
                'Aporderrt.action_loc_key', alert.action_loc_key),
            'launch-image': _Validators.check_string(
                'Aporderrt.launch_image', alert.launch_image),
        }
        if result.get('loc-args') and not result.get('loc-key'):
            raise ValueError(
                'Aporderrt.loc_key is required when specifying loc_args.')
        if result.get('title-loc-args') and not result.get('title-loc-key'):
            raise ValueError(
                'Aporderrt.title_loc_key is required when specifying title_loc_args.')
        if alert.custom_data is not None:
            if not isinstance(alert.custom_data, dict):
                raise ValueError('Aporderrt.custom_data must be a dict.')
            for key, val in alert.custom_data.items():
                _Validators.check_string('Aporderrt.custom_data key', key)
